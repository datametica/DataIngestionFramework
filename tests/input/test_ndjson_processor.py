...
class NDJSONProcessorTest(unittest.TestCase):
    def setUp(self) -> None:
        self.task = {'source': 'ndjson', 'job_name': 'ndjson-ingestion-dif-test',
    ...
    def test_NDJSON_processor_valid(self):
        with TestPipeline(options=self.pipeline_options) as p:
            data = p | NdJsonProcessor(self.task)
