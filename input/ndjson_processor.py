...
"""This module contains the implementation of a NDJSON processor that can be used to read NDJSON files.
"""
...
# Define a NdJsonProcessor class that extends the BaseSourceProcessor class
class NdJsonProcessor(BaseSourceProcessor):
    def __init__(self, prop_dict):
        self.prop_dict = prop_dict
        self.task = prop_dict

    # Define an expand method that reads data from an NDJSON file and outputs valid or invalid records
    def expand(self, p_input):
        data = (p_input
                | 'Reading data from input file' >> beam.io.ReadFromText(self.task["data_file"])
                | "Bq format" >> beam.ParDo(JsonReader(self.prop_dict)).with_outputs("valid",
                                                                                     "invalid")
                )
        return data
...
class JsonReader(beam.DoFn, ABC):
    ...
    # Define a process method that reads NDJSON data and yields valid or invalid records
