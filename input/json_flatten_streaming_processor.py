...
from dofns.parDoFunctions import addUID, FileMetaData, FileOperations, GroupMessagesByFixedWindowsContinuous, \
    WriteToGCSContinuous, FlattenJson
from input.base_source_processor import BaseSourceProcessor


class JsonFlattenStreamingProcessor(BaseSourceProcessor, abc.ABC):
    """
        ...
        columns based on a provided column name map in streaming mode.
        """
    ...
    def expand(self, p):
        ...
        read = (
                p | "Read Json From GCS Continuously." >> fileio.MatchContinuously(self.task['data_file'],
                                                                                   interval=0.5)
        ...
        (add_unique_id | "Data Preparation" >> beam.Map(lambda k: "\u0001".join(list(k[:-1])))
         | "Window into" >> GroupMessagesByFixedWindowsContinuous(300)
         | "Write Messages to GCS file" >> beam.ParDo(
                    WriteToGCSContinuous(self.task["gcs_triaging_path"]))
         )
